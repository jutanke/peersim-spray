...
public class DictGraph {
    ...
    private final List<PeerSamplingService> pssList;
    ...
    public void add(Node n, PeerSamplingService c) {
    ...
    public void addStrict(Node n, PeerSamplingService c) {
        DictNode node = new DictNode(n.getID());
        for (Node neighbor : c.getPeersThatAreAlive()) {
            node.neighbors.add(neighbor.getID());
        }
        if (this.nodes.containsKey(n.getID())) {
