 ...
 */
public abstract class CyclonProtocol implements Dynamic, Linkable, EDProtocol,
	...
	public Object clone() {
		try {
			...
			p.cache = new ArrayList<CyclonEntry>(CyclonProtocol.size);
			return p;
	...
	protected static List<CyclonEntry> remove(List<CyclonEntry> list,
			CyclonEntry entry) {
	...
	public static List<CyclonEntry> sieveOut(List<CyclonEntry> list,
			List<CyclonEntry> sieve) {
	...
	protected List<CyclonEntry> getSample(int l) {
		...
		final LinkedList<CyclonEntry> temp = new LinkedList<CyclonEntry>(
				this.cache);
		for (int i = 0; i < max; i++) {
			subset.add(temp.remove(CommonState.r.nextInt(temp.size()))
					.cyclonCopy());
		}
		return subset;
	}
	...
	protected List<CyclonEntry> getSample(int l, Node oldest) {
		...
		// for (int i = 0; i < max; i++) {
		// CyclonEntry ce =
		// temp.remove(CommonState.r.nextInt(temp.size())).cyclonCopy();
		...
		while (l > 0 && !temp.isEmpty()) {
			CyclonEntry ce = temp.remove(CommonState.r.nextInt(temp.size()))
					.cyclonCopy();
			if (ce.n.getID() != oldest.getID()) {
				subset.add(ce);
				l--;
	 ...
	 * @param sent
	 */
	protected void insertLists(final Node me, final Node destination,
			final List<CyclonEntry> received, final List<CyclonEntry> sent) {
		this.cache = insertIntoPartialView(me, destination, this.cache,
				received, sent);
	}

	public static List<CyclonEntry> insertIntoPartialView(final Node me,
		...
		if (pv.size() > size) {
			System.err.println("partial view is too big @" + me.getID()
					+ " from " + destination.getID());
	...
	public static List<CyclonEntry> insert(final List<CyclonEntry> main,
			final List<CyclonEntry> insert) {
	...
	protected void send(Node destination, CyclonMessage message) {
		...
		Transport tr = (Transport) sender.getProtocol(CyclonProtocol.tid);
		tr.send(sender, destination, message, pid);
	}

	protected boolean insert(CyclonEntry ce) {
		if (!this.contains(ce.n) && this.cache.size() < CyclonProtocol.size) {
			this.cache.add(ce);
			return true;
		}
		return false;
