...
public abstract class ScampProtocol implements Linkable, EDProtocol, CDProtocol, example.PeerSamplingService {
    ...
    protected void updateWeights(Node me) {
        ...
        for (View.ViewEntry n : this.inView.normalizeWeights()) {
            ScampMessageOld m = ScampMessageOld.updateWeightMessageInView(me, n.weight);
            this.send(me, n.node, m);
        }

        // ++ PARTIALVIEW ++
        for (View.ViewEntry n : this.partialView.normalizeWeights()) {
            ScampMessageOld m = ScampMessageOld.updateWeightMessagePartialView(me, n.weight);
            this.send(me, n.node, m);
        }

    }

    protected void send(Node me, Node destination, ScampMessageOld m) {
    ...
    public void join(Node me, Node contact) {
        ...
        if (contact != null) {
            ...
            ScampMessageOld message = new ScampMessageOld(me, ScampMessageOld.Type.Subscribe, me);
    ...
    protected void acceptSubscription(Node sender, Node subscriber) {
        ...
        ScampMessageOld message = new ScampMessageOld(sender, ScampMessageOld.Type.AcceptedSubscription, null);
