...
public class SprayTest extends TestCase {
	...
	public void testDifferentNetworkAlreadyMerged() {
		Spray s1 = new Spray();
		s1.from.add(0);
		s1.to.add(0);
		s1.to.add(1);
		...
		SprayMessage m = new SprayMessage(null, s1.from, 2, s1.to);

		s2.isFutureMerge(m);
		assertFalse(s2.mustMerge);
		assertFalse(s2.isMerge(m));

		SprayMessage r = new SprayMessage(null, s2.from, 2, s2.to);

		s1.isFutureMerge(r);
		assertFalse(s1.mustMerge);
		assertFalse(s1.isMerge(r));
	}

	public void testDifferentNetworkOneIsMergedAlready() {
		Spray s1 = new Spray();
		s1.from.add(0);
		s1.to.add(0);
		s1.to.add(1);
		...
		SprayMessage r = new SprayMessage(null, s1.from, -1, s1.to);

		s2.isFutureMerge(r);
		assertTrue(s2.mustMerge);
		assertTrue(s2.isMerge(r));
	}
}
